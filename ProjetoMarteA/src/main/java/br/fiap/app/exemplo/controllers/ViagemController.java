...
public class ViagemController {
    ...
    public String Create(@ModelAttribute("infViagem") Viagem objViagem) {
        ...
        LocalDate dataDecolagem = objViagem.getDataDecolagem();
        int duracaoEstadia = objViagem.getDuracaoEstadia();
        LocalDate dataRetorno = dataDecolagem.plusDays(duracaoEstadia + 520);

       
        objViagem.setDataRetorno(dataRetorno);
    ...
    public String edit(@PathVariable("id") Long id, @Valid @ModelAttribute("viagem") Viagem viagemAtualizada, BindingResult result) {
        if (result.hasErrors()) {
            ...
            LocalDate dataDecolagem = viagemAtualizada.getDataDecolagem();
            int duracaoEstadia = viagemAtualizada.getDuracaoEstadia();
            LocalDate dataRetorno = dataDecolagem.plusDays(duracaoEstadia + 520);

           
            viagemExistente.setDataRetorno(dataRetorno);
